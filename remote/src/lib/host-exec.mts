...
export type CaptureResult = { stdout: string; stderr: string; exitCode: number };
 ...
 */
export function json(command: string, args: string[]): Promise<string> {
...
export const hostGit = (args: string[]) => capture("git", args);

/** `gh` that never rejects — returns the exit code so callers can distinguish
 *  "no PR" (exit 1) from a real read. */
export const ghCapture = (args: string[]) => capture("gh", args);

/** `gh` read that throws on a non-zero exit (for the `--json` reads). */
export const ghJson = (args: string[]): Promise<string> => json("gh", args);
