 ...
 * throw from here.
 */
export async function syncMain(git = hostGit) {
 ...
 */
export async function resumeFromOrigin(branch, git = hostGit) {
 ...
 */
export async function logSince(branch, base = "origin/main", git = hostGit) {
 ...
 */
export async function push(branch, git = hostGit) {
 ...
 */
export async function pushCheckpoint(branch, git = hostGit) {
    const result = await git(["push", "-u", "origin", branch]);
    if (result.exitCode !== 0) {
 ...
 */
export async function commitOnBranch(sandbox, message, opts = {}) {
    const trailer = opts.trailer ? ` --trailer ${shellQuote(opts.trailer)}` : "";
    const result = await sandbox.exec(`git ${BOT_IDENTITY} commit --allow-empty -m ${shellQuote(message)}${trailer}`);
    if (result.exitCode !== 0) {
 ...
 */
export async function dropArtifacts(sandbox, files) {
