...
import { commitOnBranch, dropArtifacts, logSince, push, pushCheckpoint, } from "../lib/branch.mjs";
...
import { ensureTaskList, runChecklistLoop } from "../lib/task-loop.mjs";
import { describeRun, forwardedEnvKeys } from "../lib/profiles.mjs";
import { admit, } from "../lib/issue-intake.mjs";
import { assertKnownFlags, readFlag } from "../lib/cli.mjs";
import { isEntrypoint } from "../lib/entrypoint.mjs";
import { openRun } from "../lib/run.mjs";
import { renderConventions } from "../lib/toolchains.mjs";
import { readSandboxFile } from "../phases/context.mjs";
 ...
 */
export async function runIssue(config, run, issueNumber, issue, issueSource, runDeps) {
...
export async function main(options, deps) {
    ...
    if (admission.kind === "rejected") {
        ...
        await issueSource.comment(options.issue, admission.detail).catch((commentError) => {
            console.error(`${COMMENT_FAILURE[admission.because]} on the issue: ${errorMessage(commentError)}`);
        });
