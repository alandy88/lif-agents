...
import { hostGit } from "../lib/host-exec.mjs";
import { githubIssueSource, issueIsEpic, } from "../lib/github-issue.mjs";
import { checkOffTask, parseTaskDoneTrailers, renderTaskList, stripTaskSection, taskDoneTrailer, } from "../lib/task-list.mjs";
import { ensureTaskList, runTaskLoop } from "../lib/task-loop.mjs";
import { describeRun, forwardedEnvKeys, resolvePhases, } from "../lib/profiles.mjs";
import { readFlag } from "../lib/cli.mjs";
...
import { runDeliverPhase } from "../phases/deliver.mjs";
export { readFlag };
...
const BOT_IDENTITY = `-c user.name='sandcastle-agent[bot]' ` +
 ...
 */
export const NO_NOTES_PLACEHOLDER = "(No deviations logged yet — nothing so far has forced a departure from the plan.)";
 ...
 */
export function renderPrBody(issueNumber, run, summary) {
 ...
 */
export async function runIssue(config, run, issueNumber, issue, issueSource) {
    ...
    try {
        ...
        if (tasks.length > MAX_TASKS) {
            throw new Error(`Issue #${issueNumber} has ${tasks.length} checklist tasks (cap ${MAX_TASKS}); split it into smaller issues.`);
        }
        const result = await runTaskLoop(tasks, done, {
            ...
            recordDone: async (index) => {
                ...
                await sandbox.exec(`git ${BOT_IDENTITY} ` +
        ...
        if (result.kind === "stuck") {
            ...
            const detail = `Task ${result.taskIndex}/${tasks.length} ("${stuckTask.text}") made no commits after a retry. ` +
        ...
        console.log(`Tasks: ${result.completed.length} built, ${result.skippedDone.length} resumed; ` +
...
export async function main(options, deps) {
    ...
    if (issue.state === "CLOSED") {
        ...
        await issueSource.comment(options.issue, detail).catch((commentError) => {
            console.error(`Could not report closed-issue error on the issue: ${errorMessage(commentError)}`);
        });
        throw new Error(detail);
    }
    if (labelTriggered && !issue.labels.includes("ready-for-agent")) {
        console.log(`Issue #${options.issue} no longer has the ready-for-agent label (removed while queued) — skipping.`);
        return "skipped";
    }
    if (await deps.issueIsEpic(options.issue)) {
        const detail = `Issue #${options.issue} has native GitHub sub-issues (it's an epic); the sandcastle-agent run ` +
    ...
    try {
        ...
        console.error(detail);
        await issueSource.comment(options.issue, detail).catch((commentError) => {
            console.error(`Could not report configuration error on the issue: ${errorMessage(commentError)}`);
        });
        throw error;
    }
    ...
    if (deps.env.GITHUB_ACTIONS === "true") {
        writeFileSync(".sandcastle/.env", forwardedEnvKeys(Object.values(run.phases))
            .map((key) => `${key}=\n`)
            .join(""));
    }
 ...
 * one file.
 */
export function runImplementLoop(config, argv) {
    return main(parseCli(argv), {
        issueSource: githubIssueSource,
        issueIsEpic,
        runIssue: (run, issueNumber, issue, issueSource) => runIssue(config, run, issueNumber, issue, issueSource),
        env: process.env,
    });
}
export { isEntrypoint };
//# sourceMappingURL=implement.mjs.map