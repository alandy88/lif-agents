 ...
 */
export async function runIteration(config, run, next) {
    ...
    try {
        ...
        const taskCtx = { ...shared, agent: createAgent(run.phases.task) };
        const verifyCtx = { ...shared, agent: createAgent(run.phases.review) };
        ...
        return runDeliverPhase({
            branch,
            title: `Task ${label}`,
            body: `Automated sandcastle-agent run (${describeRun(run)}): ` +
...
export function runTaskLoop(config, argv) {
    return main(parseCli(argv), {
        syncMain: async () => (await hostGit(["pull", "--ff-only", "origin", "main"])).exitCode === 0,
